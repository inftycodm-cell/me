...
def apply_win11_chrome(tk_window):
  ...
  if os.name != 'nt':
    ...
    try:
      ...
      DWMSBT_MAINWINDOW = 2
      dark = ctypes.c_int(1)
      dwmapi.DwmSetWindowAttribute(hwnd,var2,ctypes.byref(dark),ctypes.sizeof(dark))
      corner = ctypes.c_int(DWMWCP_ROUND)
      dwmapi.DwmSetWindowAttribute(hwnd,var3,ctypes.byref(corner),ctypes.sizeof(corner))
      backdrop = ctypes.c_int(DWMSBT_MAINWINDOW)
      dwmapi.DwmSetWindowAttribute(hwnd,var4,ctypes.byref(backdrop),ctypes.sizeof(backdrop))
      return None
    except Exception:
      return None
...
'''
  ...
  def show_history_page(self):
    ...
    history = var10(merged.values(),key=lambda h: (h.get('activated_at') or ''),reverse=True)
    merged
...
Cr b Termin''',font=ctk.CTkFont('Segoe UI',10,'bold'),text_color='#FFFFFF',justify='left',anchor='w').pack(side='left',anchor='w')
  ...
  def show_update_window(self,latest_version,download_url,force=False,changelog=''):
    ...
    if getattr(self,'update_win',None) is not None and self.update_win.winfo_exists():
      ...
      def start_download():
        ...
        threading.Thread(target=self.var13,args=(download_url,latest_version),daemon=True).start()
  ...
  def var13(self,url,latest_version):
    ...
    if (self.version_ok and self.license_active):
      ...
      if starting:
        ...
        def work():
          ...
          if (self.var5 and self.auto_burger_click_stop.is_set()):
            pass

          if getattr(self,'_closing',False) and self.license_active:
            for x,y in points:
              if (self.var5 and (self.auto_burger_click_stop.is_set() or getattr(self,'_closing',False) or self.license_active)):
                self.var5 = False
                try:
                  self.after(0,self.refresh_starts)
                  return None
...
Place Graff.png next to Shadow.py / the .exe''')
  ...
  def var17(self,img_path):
    ...
    self.var18(ov)
    try:
      ov.after(50,lambda : self.var18(ov))
      ov.after(250,lambda : self.var18(ov))
    except Exception:
      pass
    ...
    ov.protocol('WM_DELETE_WINDOW',self.var16)
    return None
    e = None
    del(e)
  ...
  def _get_marijuana_path(self):
    runtime = os.path.join(self.var19(),'Marijuana.exe')
    if os.path.isfile(runtime):
      return runtime
    else:
      src_dir = self._marijuana_source_dir()
...
if __name__ == '__main__':
  ...
  alive = (bool(app.var1()) and getattr(app,'_closing',False))
  if alive:
...
# var1 = 5f696e7374616c6c5f6d697373696e675f646570656e64656e63696573
# var2 = 44574d57415f5553455f494d4d4552534956455f4441524b5f4d4f4445
# var3 = 44574d57415f57494e444f575f434f524e45525f505245464552454e4345
# var4 = 44574d57415f53595354454d4241434b44524f505f54595045
# var5 = 6175746f5f6275726765725f636c69636b5f72756e6e696e67
# var6 = 5f73746172745f676c6f62616c5f64656c6574655f6c697374656e6572
# var7 = 6c697374
# var8 = 66657463685f7365727665725f6c6963656e73655f686973746f7279
# var9 = 5f636c6f73655f616c6c5f73657474696e67735f70616e656c73
# var10 = 736f72746564
# var11 = 7570646174655f6c6963656e73655f706167655f737461747573
# var12 = 7475706c65
# var13 = 5f646f776e6c6f61645f616e645f6170706c795f757064617465
# var14 = 5f73746172745f636c69636b65725f6c696b655f646f75626c655f636c69636b
# var15 = 5f6275696c645f7374617475735f73657474696e67735f626c6f636b
# var16 = 636c6f73655f67726166665f66696e6465725f6f7665726c6179
# var17 = 73686f775f67726166665f66696e6465725f6f7665726c6179
# var18 = 5f6170706c795f636c69636b7468726f7567685f7374796c6573
# var19 = 5f6765745f6d6172696a75616e615f72756e74696d655f646972
# var20 = 5f73746172745f6d6172696a75616e615f6c696b655f646f75626c655f636c69636b